...
export const REGISTRY: Record<string, Category> = {
	entrance: {
		...
		animations: {
			...
			'slide-in-blurred': {
				label: 'Slide In Blurred',
				configs: slideInBlurred,
			},
			'slide-in-elliptic': {
				label: 'Slide In Elliptic',
				configs: slideInElliptic,
			},
	...
	exit: {
		...
		animations: {
			...
			'rotate-out-max': {
				label: 'Rotate Out Max',
				configs: rotateOutMax,
			},
			'slide-out': { label: 'Slide Out', configs: slideOut },
			'slide-out-fwd': {
				label: 'Slide Out Forward',
				configs: slideOutFwd,
			},
			'slide-out-bck': { label: 'Slide Out Back', configs: slideOutBck },
			'slide-out-blurred': {
				label: 'Slide Out Blurred',
				configs: slideOutBlurred,
			},
			'slide-out-elliptic': {
				label: 'Slide Out Elliptic',
				configs: slideOutElliptic,
			},
		},
	},
	attention: {
	...
	text: {
		...
		animations: {
			...
			'text-shadow-drop': {
				label: 'Text Shadow Drop',
				configs: textShadowDrop,
			},
			'text-shadow-pop': {
				label: 'Text Shadow Pop',
				configs: textShadowPop,
			},
	...
	basic: {
		...
		animations: {
			...
			'shadow-drop-max': {
				label: 'Shadow Drop Max',
				configs: shadowDropMax,
			},
...
export function buildClassIndex(): Record<
	string,
	{ category: string; animation: string }
> {
	const index: Record<string, { category: string; animation: string }> = {};
	for (const [categoryId, category] of Object.entries(REGISTRY)) {
		for (const [animationId, group] of Object.entries(
			category.animations
		)) {
			for (const cls of Object.keys(group.configs)) {
				if (!(cls in index)) {
